...
export class VideoProcessor {
  ...
  constructor() {
    ...
    this.fontPath =
      ...
      (process.platform === 'darwin'
        ...
        : '/usr/share/fonts/custom/Arial.ttf');
  ...
  async stitchVideos(mediaFiles, event, onProgress) {
    ...
    try {
      ...
      onProgress?.(0.1);
      const tempFiles = await this.convertToConsistentFormat(
        mediaFiles,
        onProgress
      );
  ...
  async convertToConsistentFormat(mediaFiles, onProgress) {
    const tempFiles = [];
    ...
    for (let i = 0; i < mediaFiles.length; i++) {
      ...
      if (mediaFile.mediaType === 'audio') {
        ...
        await this.convertAudioToVideo(
          mediaFile.path,
          tempFile,
          mediaFile.guestName
        );
        ...
        await this.convertVideoToConsistentFormat(
          mediaFile.path,
          tempFile,
          mediaFile.guestName
        );
  ...
  async convertAudioToVideo(inputPath, outputPath, guestName) {
    ...
    console.log(
      JSON.stringify(
        {
          message:
            'Converting audio to video with custom waveform and guest name',
          guestName,
  ...
  async convertVideoToConsistentFormat(inputPath, outputPath, guestName) {
    ...
    console.log(
      JSON.stringify(
        {
          message:
            'Converting video with guest name overlay and black frame trimming',
          guestName,
          trimStart,
        },
        null,
    ...
    try {
      ...
      await Promise.all(
        mediaFiles.map(async mediaFile => {
          try {
            ...
            console.log(
              JSON.stringify(
                { message: 'Cleaned up file', file: mediaFile.path },
                null,
                4
              )
            );
      ...
      if (outputPath) {
        try {
          ...
          console.log(
            JSON.stringify(
              { message: 'Cleaned up output file', outputPath },
              null,
              4
            )
          );
