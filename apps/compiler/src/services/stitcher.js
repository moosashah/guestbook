...
export class CompilerService {
  ...
  async compileEvent(eventId, webhookUrl) {
    try {
      ...
      console.log(
        JSON.stringify(
          { message: 'Fetching event and messages', eventId },
          null,
          4
        )
      );
      ...
      if (event.data.final_video_key) {
        throw new Error(
          `Final video already exists for event: ${eventId}. Use the existing video or delete it first.`
        );
      }

      // Fetch all messages for the event
      const messages = await this.MessageEntity.query
      ...
      const { s3Key, s3Url } = await this.s3Service.uploadFinalVideo(
        outputPath,
        eventId
      );
      ...
      await this.EventEntity.patch({ id: eventId })
        .set({ final_video_key: s3Key })
        .go();
  ...
  async getCompilationStatus(eventId) {
    return (
      this.compilationStatuses.get(eventId) || {
        eventId,
        status: 'pending',
      }
    );
