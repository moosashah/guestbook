...
const qrCodeDownloadCommand = (key: string) =>
...
export const completeMultipartUpload = async (
  ...
  return {
    Location:
      response.Location ||
      `https://${BUCKET_NAME}.s3.eu-west-2.amazonaws.com/${key}`,
...
export const multipartUpload = async (
  ...
  console.log(
    `[multipart-upload] Starting multipart upload for ${key}, size: ${fileSize} bytes`
  );
  ...
  try {
    for (let partNumber = 1; partNumber <= totalParts; partNumber++) {
      ...
      console.log(
        `[multipart-upload] Uploading part ${partNumber}/${totalParts}, size: ${partBuffer.length} bytes`
      );
    ...
    console.log(
      `[multipart-upload] Successfully completed multipart upload for ${key}`
    );
  } catch (error) {
    console.error(
      `[multipart-upload] Error during multipart upload for ${key}:`,
      error
    );
...
export const qrCodeUpload = async (key: string, qrCodeBuffer: Buffer) =>
  await s3Client().send(
    s3UploadCommand(key, qrCodeBuffer, 'image/png', 'public, max-age=31536000')
  );

// Final video operations
export const getFinalVideoDownloadUrl = async (
  key: string,
  eventId: string
) => {
...
export const getBannerImageUrl = async (
  key: string,
...
export const getWelcomeMessageUrl = async (
  key: string,
  expiresIn: number = 3600
) => {
  const command = new GetObjectCommand({
