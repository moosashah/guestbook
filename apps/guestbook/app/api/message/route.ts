...
export async function POST(req: NextRequest) {
  try {
    ...
    const contentType =
      validatedMediaType === 'video' ? 'video/webm' : 'audio/webm';
    ...
    console.log(
      `[message] Uploading ${validatedMediaType} message, size: ${buffer.length} bytes`
    );
    console.log(
      `[message] Will use ${shouldUseMultipartUpload(buffer.length) ? 'multipart' : 'regular'} upload`
    );
    ...
    await EventEntity.patch({ id: validatedEventId })
      .add({ message_count: 1 })
      .go();
