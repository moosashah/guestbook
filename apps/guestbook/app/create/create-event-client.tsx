...
export default function CreateEventClient({ user }: CreateEventClientProps) {
    ...
    const onSubmit = async (data: FormValues) => {
        ...
        try {
            ...
            eventFormData.append("submission_start_date", data.dateRange.from!.toISOString());
            eventFormData.append("submission_end_date", data.dateRange.to!.toISOString());
            ...
            if (welcomeMessageBlob) {
                eventFormData.append("welcome_message", welcomeMessageBlob, "welcome-message.webm");
            }
            ...
            alert(`Failed to create event. Please try again. Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
        } finally {
        ...
        <div className="container mx-auto py-8 px-4 max-w-2xl">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                            ...
                            render={({ field }) => (
                                ...
                                >
                                    {Object.entries(packageFeatures).map(([pkg, details]) => (
                                        <div
                                            key={pkg}
                                            className={`border rounded-lg p-4 transition-colors ${selectedPackage === pkg
                                            ...
                                            <div className="flex items-center space-x-3">
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex items-center justify-between">
                                                        <Label htmlFor={pkg} className="text-lg font-medium capitalize">
                                                            {pkg}
                                                        </Label>
                ...
                <div className="flex justify-end">
                    ...
                    >
                        {isSubmitting ? "Creating..." : `Create Event - $${packagePrices[selectedPackage]}`}
                    </Button>
                </div>
            </form>
        </div>
