...
export const useMediaPlayer = ({ messageId, eventId }: UseMediaPlayerProps) => {
    ...
    const fetchMediaUrl = useCallback(async () => {
        ...
        try {
            console.log(`[use-media-player] Fetching media URL for message: ${messageId}`);
            ...
            console.log(`[use-media-player] Successfully fetched media URL for message: ${messageId}`);

            setMediaData(data);
            return data;
        } catch (err) {
            const errorMessage = err instanceof Error ? err.message : "Failed to fetch media URL";
